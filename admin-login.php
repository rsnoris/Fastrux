...
<head>
  ...
  <title>Admin Portal Login — Fastrux Logistics</title>
...
<body>
  ...
  <nav class="mobile-menu" id="mobileMenu">
    ...
    <div class="header-actions" style="margin-top:8px;">
      <a class="btn btn-outline" href="login">General Login</a>
    </div>
  </nav>
  ...
  <div class="auth-wrapper" style="align-items:flex-start;padding-top:48px;">
    <div style="width:100%;max-width:460px;margin:0 auto;">
      ...
      <div class="auth-card" style="margin-bottom:20px;">
        ...
        <p class="auth-subtitle">Sign in to manage users, approve accounts, and oversee the Fastrux platform.</p>

        <div class="form-feedback" id="loginFeedback"></div>
        <form id="loginForm" novalidate>
          <input type="hidden" name="form_type" value="login" />
          <input type="hidden" name="expected_role" value="admin_portal" />
          <div class="form-group">
            <label for="email">Email or Username</label>
            <input class="form-control" type="text" id="email" name="email"
                   placeholder="admin@fastrux.com or Admin" required autocomplete="username" />
          </div>
          <div class="form-group">
            <label for="password">
              Password
        ...
        <p class="auth-footer-text" style="font-size:13px;color:var(--muted-foreground);">
          Not an admin? <a href="login">General login</a>
        </p>
      </div>

      <!-- Features card -->
      <div class="auth-card" style="padding:24px;">
        <h2 style="font-size:16px;font-weight:700;margin-bottom:4px;">Admin Portal Capabilities</h2>
        <p style="font-size:13px;color:var(--muted-foreground);margin-bottom:0;">Manage and monitor the entire Fastrux platform from one place.</p>
        <div class="features-list">
          <div class="feature-tile">
            <iconify-icon icon="lucide:users"></iconify-icon>
            <span class="ft-title">User Management</span>
            <span class="ft-desc">View, approve, reject, and manage all platform users.</span>
          </div>
          <div class="feature-tile">
            <iconify-icon icon="lucide:shield-check"></iconify-icon>
            <span class="ft-title">Staff Approvals</span>
            <span class="ft-desc">Approve or reject corporate staff and driver account requests.</span>
          </div>
          <div class="feature-tile">
            <iconify-icon icon="lucide:bar-chart-2"></iconify-icon>
            <span class="ft-title">Platform Analytics</span>
            <span class="ft-desc">Monitor loads, payments, and platform-wide activity in real time.</span>
          </div>
          <div class="feature-tile">
            <iconify-icon icon="lucide:settings"></iconify-icon>
            <span class="ft-title">Role Control</span>
            <span class="ft-desc">Assign roles and create additional admin accounts (Super-Admin only).</span>
          </div>
        </div>
      </div>
  ...
  <script>
    ...
    // Pre-fill identifier from URL query param
    ...
    document.getElementById('loginForm').addEventListener('submit', async function(e) {
      ...
      try {
        ...
        if (data.success) {
          ...
          // Enforce portal — only admin / super_admin may use this page
          if (role !== 'admin' && role !== 'super_admin') {
            feedback.className   = 'form-feedback error';
            feedback.textContent = '✗ This portal is for admins only. Please use the general login page.';
          ...
          const redirect = params.get('redirect') || 'admin-dashboard.php';
